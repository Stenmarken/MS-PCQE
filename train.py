...
def main(config):
    ...
    for split in range(config.split_num):  # range(config.split_num):   range(config.split_num-1, -1, -1):  range(1, 5)----1 to 4
        # model
        ...
        # optimizer
        optimizer = optim.Adam(model.parameters(), lr = config.conv_base_lr, weight_decay = 1e-4)  # 0.0000001  1e-4
        ...
        if config.database == 'SJTU':
            ...
            images_dir_05 = '../rotation/wpc_scale_0.5_512_12/frames_wpc'
        ...
        for epoch in range(config.epochs):
            ...
            tqdm_train = tqdm(train_loader, ncols=80)
            for i, (frames_dir_10_video, frames_dir_05_video, frames_dir_10_video_mask, frames_dir_05_video_mask, labels) in enumerate(tqdm_train):

                labels = labels.to(device)
                y_mid_ref = model(frames_dir_10_video, frames_dir_05_video, frames_dir_10_video_mask, frames_dir_05_video_mask, mode_idx=0)

                optimizer.zero_grad()
                loss = criterion(y_mid_ref.float(), labels.float())
            ...
            print('The current learning rate is {:.09f}'.format(lr[0]))
            scheduler.step()
            ...
            use_num = config.sampling_div * 1
            outputs_tem_ref = torch.zeros(use_num)
            cof_num = torch.zeros(use_num)
            outputs_tem = torch.zeros(use_num)
            ...
            with torch.no_grad():
                ...
                if (epoch+1) == config.epochs:
                    torch.save(model.state_dict(), config.ckpt_path + '/' + config.model_name +'_' + config.database +'_' + str(split+1) + '_' + 'last.pth')
